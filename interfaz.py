...
sistema.py (Biblioteca, Libro, Usuario y el patron Strategy de recomendacion).
Separar la interfaz de la logica es una buena practica de diseno: si manana
cambia la interfaz, las clases no se tocan.
...
class App(ctk.CTk):
    def __init__(self):
        ...
        # Usuario "de sesion" para registrar los prestamos
        self.usuario = Usuario("Invitado", "invitado@biblioteca.com", "1234")
        self._precargar_libros()   # algunos libros de ejemplo (se pueden borrar)
    ...
    def _construir_sidebar(self):
        ...
        ctk.CTkLabel(self.sidebar, text="Gestion de Biblioteca\nProgramacion Avanzada",
                     ...
                     justify="left").grid(row=6, column=0, padx=20, pady=18, sticky="sw")
    ...
    def mostrar_catalogo(self):
        self._limpiar()
        disponibles = self.biblioteca.cantidad_disponible()
        total = len(self.biblioteca.catalogo())
        self._titulo(f"Catalogo  ({disponibles}/{total} disponibles)")
    ...
    def _tarjeta_libro(self, padre, libro, fila):
        ...
        ctk.CTkLabel(card, text=info, font=ctk.CTkFont(size=15, weight="bold"),
        ...
        estado_txt = "Prestado" if prestado else "Disponible"
        estado_col = WARN_COLOR if prestado else OK_COLOR
        ctk.CTkLabel(card, text=estado_txt, font=ctk.CTkFont(size=12, weight="bold"),
                     text_color=estado_col).grid(row=0, column=1, rowspan=2, padx=10)
    ...
    def mostrar_agregar(self):
        ...
        for campo in ["Titulo", "Autor", "Genero", "Editorial"]:
            ctk.CTkLabel(form, text=campo, anchor="w").grid(
        ...
        ctk.CTkButton(form, text="Agregar al catalogo", height=40,
                      fg_color=ACCENT, hover_color=ACCENT_HOVER,
                      command=self.accion_agregar).grid(
            sticky="ew", padx=20, pady=22)
    ...
    # ============ VISTA: recomendar (patron Strategy) ============
    def mostrar_recomendar(self):
        ...
        ctk.CTkButton(panel, text="Buscar recomendacion", height=40,
                      fg_color=ACCENT, hover_color=ACCENT_HOVER,
                      command=self.accion_recomendar).grid(
    ...
    def accion_recomendar(self):
        ...
        if not pref:
            ...
            return
        # Patron Strategy: elegimos la estrategia segun el criterio
